...
const playSound = audio => {
  const clone = audio.cloneNode();
  clone.play();
  window.setTimeout(() => (clone.volume = 0.8), 400, null);
  window.setTimeout(() => (clone.volume = 0.6), 800, null);
  window.setTimeout(() => (clone.volume = 0.4), 1200, null);
  window.setTimeout(() => (clone.volume = 0.2), 1600, null);
  window.setTimeout(() => (clone.volume = 0), 2000, null);
};
